...
st.sidebar.write("""#### Team members: Nguyễn Văn A & Trần Thị B""")
st.sidebar.write("""#### Instructor: """)
st.sidebar.write("""#### Date: 12/2024""")

if choice == 'Business Objective':
    # Submenu for 'Business Objective' to switch between tabs
    objective_choice = st.radio("Choose a section:", options=["Overview", "Product Information", "Algorithm"])

    if objective_choice == "Overview":
        st.subheader("Sơ lược về Hasaki")
        # Display banner image
        banner = Image.open('hasaki_banner.jpg')
        st.image(banner, caption="Sơ lược về Hasaki", use_column_width=True)
        ...
        st.image(product_management_image, caption="Quản lý sản phẩm thông qua đánh giá ý kiến khách hàng", use_column_width=True)
        ...
        st.image(sentiment_image, caption="Sentiment Analysis", use_column_width=True)
    ...
    elif objective_choice == "Product Information":
        st.subheader("Product Information")
        # Input field for entering product code
        product_code_input = st.text_input("Enter product code:")
        ...
        product_code_select = st.selectbox("Or select a product code from the list:", product_codes)
    ...
    sns.histplot(data['so_sao'], kde=True)
    st.pyplot()
    ...
    # Move confusion matrix display here at the end
    st.write("###### Confusion matrix:")
    st.code(cm)

    # Plot confusion matrix using seaborn heatmap only once
